...
class CreatePMFile:
    ...
    def prepare_xml_file(self, active_nssai, config_data):
        ...
        for k, v in dict(active_nssai).items():
            for i in range(len(v)):
                ...
                pm_meas_value_r.text = self.get_registered_sub_nbr_mean(config_data, v[i])
    ...
    def get_registered_sub_nbr_mean(self, config_data, snssai_id):
        '''
            ...
            for i in range(len(v)):
                if 'maxNumberofUEs' in dict(v[i]):
                    if dict(v[i])['s-nssai'] == snssai_id:
                        self.logger.info("Inside get_RegisteredSubNbrMean method & \
                        maxNumberofUEs found in slice profile for " + snssai_id)
                        maxNumberofUEs = dict(v[i])['maxNumberofUEs']
                        random_int = random.randint(100, int(maxNumberofUEs))
                        return str(random_int)
                else:
                    self.logger.info("Inside get_RegisteredSubNbrMean method & "
                                     "maxNumberofUEs NOT-Found in slice profile")
