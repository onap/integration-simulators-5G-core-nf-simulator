...
def ves_agent_call(filename, application_config):
    ...
    collector_port = application_config.dcae_collector_port.replace(",","")
    collector_ip = application_config.dcae_collector_ip.replace(",","")
...
def simulate_smf(application_config, param):
    '''
...
if __name__ == '__main__':
    ...
    logger.info('SMF Application started')

    if sys.argv[1] == 'SMF':
        while 1:
            with open('/etc/config/smf-conf/granularity_period.txt') as mytxt:
                for line in mytxt:
            ...
            simulate_smf(application_config, sys.argv[1])
